...
public class KartController : MonoBehaviour
{
    ...
    {
        kart.GetComponent<MeshRenderer>().material = otherMaterial;
    }

    void Update()
    {
        if (!enabled || !pv.IsMine) return;
        ...
        if (Input.GetButtonDown("Jump") && !drifting)
        {
            if (Input.GetAxis("Horizontal") != 0)
            {
                ...
                GetComponent<PhotonView>().RPC("MainParticles", RpcTarget.AllViaServer);
            }
    ...
    [PunRPC]
    public void SecondaryParticles(Vector3 color)
    {
    ...
    public void Boost()
    {
        ...
        if (driftMode > 0)
        {
            float duration = .5f * driftMode;
            DOVirtual.Float(currentSpeed * 4, currentSpeed, duration, Speed);
            DOVirtual.Float(0, 2, .5f, ChromaticAmount)
                .OnComplete(() => DOVirtual.Float(2, 0, duration, ChromaticAmount));
            GetComponent<PhotonView>().RPC("BoostRPC", RpcTarget.AllViaServer);
        }
        ...
        GetComponent<PhotonView>().RPC("MainParticlesStop", RpcTarget.AllBuffered);
    ...
    void PlayFlashParticle(Color c)
    {
        ...
        GetComponent<PhotonView>().RPC("SecondaryParticles", RpcTarget.AllViaServer, new Vector3(c.r, c.g, c.b));
    }
